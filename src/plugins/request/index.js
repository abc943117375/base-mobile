    ...
    config.forEach((item) => {
      ...
      Object.defineProperty(this.api, `${namespace}${sep}${item.name}`, {
        value: (outerParams, outerOptions = {}) => {
          ...
          const dataParams = Object.assign( // 向请求中全局添加两个参数
            {
          ...
          if (config.headers) { // 如果定义了数据结构
            config.headers['X-Token'] = token
          } else {
            config.headers = { 'X-Token': token }
          }
          if (outerOptions && outerOptions.isExteriorAPI) { // 如果是注销操作,删除token
...
function _normoalize(options, params) {
  ...
  if (options.method === 'post') { // 如果请求方式为post
    options.data = params // 参数放在data中
  } else if (options.method === 'get') { // 如果请求方式为params  
